...
```{python}
#| echo: false
fig = px.line(df, x="Date", y=["IPC(idx)", "Point d'indice(idx)"], title="Joint evolution of inflation and point d'indice", color_discrete_sequence=px.colors.qualitative.D3)
fig.update_xaxes(range=[init_date, df['Date'][-1]])
...
fig = px.line(df, x="Date", y=["Point d'indice(euros constant)"], title="Point d'indice (€ corrigé de l'inflation): <br> Réference 1 le " + init_date,color_discrete_sequence=px.colors.qualitative.D3)
fig.update_xaxes(range=[init_date, df['Date'][-1]])
