...
deltax = maxval_x - minval_x
deltay = maxval_y - minval_y
...
sub1.set_ylim([minval_x - deltax / 5, 1.2 * maxval_x + deltax / 5])
sub1.set_xlim([minval_y - deltay / 5, 1.2 * maxval_y + deltay / 5])
...
def computer_mid_iter(points, t=0.5, closed=True):
...
def plot_trajs_points(points, ax=sub1, t=0.5, lw=2, closed=True):
    list_traj, end_points = computer_mid_iter(points, t=t, closed=closed)
    # print(list_traj)
    for i, points in enumerate(list_traj):
        ...
        plot_points(points, ax=sub1, color=color_blind_list[0],
                    alpha=1 - i / len(list_traj))
    return end_points


t_vals = np.linspace(0, 1, num=48)
saving = True
closed = False
files = ''
#
...
for image_nb, t in enumerate(t_vals):
    ...
    end_points = plot_trajs_points(points, ax=sub1, t=t, lw=1, closed=closed)
    # print(end_points)
